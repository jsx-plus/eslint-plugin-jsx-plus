...
module.exports = {
  meta: {
    docs: {
      description: 'x-elseif should have a x-if or x-elseif ahead',
    ...
    schema: [],
    messages: {
      oneIfStatementNeededAhead: `x-elseif should have a x-if or x-elseif ahead`
    }
  },
  ...
  create: function(context) {
    return {
      JSXElement(node) {
        let attrProp;
        const hasAttr = hasProp(node.openingElement.attributes, X_ELSEIF);
        if (!hasAttr) return;

        attrProp = getProp(node.openingElement.attributes, X_ELSEIF);
